...
def list_users():
    if is_database_available():
        ...
        return [_serialize_db_user(user) for user in User.objects.all()]
...
def create_user(data):
    if is_database_available():
        ...
        if data.get('_pending_upload') is not None:
            ...
            user.save(update_fields=['avatar', 'avatar_key', 'updated_at'])
    ...
    if is_database_available():
        ...
        if partial:
            ...
            if 'avatar_key' in data:
                ...
                fields.append('avatar_key')
            fields.extend(['name', 'email'])

        if fields:
            user.save(update_fields=list(dict.fromkeys(fields + ['updated_at'])))
        return _serialize_db_user(user)
